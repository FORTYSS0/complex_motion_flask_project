...
def generate_all_plots(data):
    """Создаёт статичные PNG‑графики для экспорта (все 5 графиков)."""
    # ---- 1. Траектория (рис.1) ----
    ...
    # ---- 4. Траектория + векторы скоростей (рис.2a) ----
    generate_static_trajectory_with_velocities(data)

    # ---- 5. Траектория + векторы ускорений (рис.3a) ----
    generate_static_trajectory_with_accelerations(data)


def generate_static_trajectory_with_velocities(data):
    """Статичная PNG‑версия графика 2a: траектория + векторы скоростей (масштабированные)."""
    t_vals = np.linspace(0, 2.5, 200)
    x_t = 8 * np.cos(np.pi * t_vals**2 / 3)
    y_t = 16 * np.sin(np.pi * t_vals**2 / 3)
    z_t = 2 * t_vals**2 + 4
    ...
    fig = plt.figure(figsize=(8, 6))
    ax = fig.add_subplot(111, projection='3d')
    ax.plot(x_t, y_t, z_t, label='Траектория', color='blue', linewidth=2)
    ax.scatter(data['point'][0], data['point'][1], data['point'][2], color='red', s=50, label='M (t=1)')
    ax.set_xlabel("X'")
    ax.set_ylabel("Y'")
    ax.set_zlabel("Z'")
    ax.set_title('Траектория и векторы скоростей (исходная система)')
    ...
    draw_axes(ax, origin=(0,0,0), length=axis_len, color='black', labels=["X'", "Y'", "Z'"])
    draw_axes(ax, origin=O, length=axis_len, color='red', labels=['X', 'Y', 'Z'])
    ax.scatter(O[0], O[1], O[2], color='blue', s=40, label='O')
    ax.text(O[0], O[1], O[2], ' O', color='blue', fontsize=10)
    ...
    if len_arc > 0:
        u_arc = -dx_arc / len_arc * 0.3 * radius
        v_arc = -dy_arc / len_arc * 0.3 * radius
        ax.quiver(x_arc[0], y_arc[0], z_arc, u_arc, v_arc, 0, color='green', arrow_length_ratio=0.2)
    ax.text(x_arc[len(theta)//2], y_arc[len(theta)//2], z_arc+0.02, f'ω = {abs(data["omega"]):.2f}', color='green', fontsize=9)
    ...
    for vec, col, lab in zip(vectors, colors, labels):
        ...
        if norm > 1e-8:
            unit = vec / norm
            scaled = unit * scale_len
        else:
            scaled = np.zeros(3)
        ax.quiver(data['point'][0], data['point'][1], data['point'][2],
                  scaled[0], scaled[1], scaled[2],
                  color=col, label=lab, arrow_length_ratio=0.2, linewidth=2)
        end = np.array(data['point']) + scaled
        ax.text(end[0], end[1], end[2], lab, color=col, fontsize=8, ha='center', va='center')
...
def generate_static_trajectory_with_accelerations(data):
    """Статичная PNG‑версия графика 3a: траектория + векторы ускорений (масштабированные)."""
    t_vals = np.linspace(0, 2.5, 200)
    x_t = 8 * np.cos(np.pi * t_vals**2 / 3)
    y_t = 16 * np.sin(np.pi * t_vals**2 / 3)
    z_t = 2 * t_vals**2 + 4
    ...
    ax.set_ylabel("Y'")
    ax.set_zlabel("Z'")
    ax.set_title('Траектория и векторы ускорений (исходная система)')
...
# (остаются без изменений, они уже были в предыдущих версиях)
# ... (здесь должны быть все функции generate_interactive_...)
